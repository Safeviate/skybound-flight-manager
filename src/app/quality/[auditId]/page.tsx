...
import { AlertTriangle, CheckCircle, ListChecks, MessageSquareWarning, Ban, MinusCircle, XCircle, FileText, Save, Send, PlusCircle, Check, Percent, Bot, Printer, ArrowLeft, Signature, Eraser, Camera, Image as ImageIcon, RotateCw, FileUp, Trash2, ChevronDown } from 'lucide-react';
...
const AuditReportView = ({ audit, onUpdate, personnel, onNavigateBack }: { audit: QualityAudit, onUpdate: (updatedAudit: QualityAudit, showToast?: boolean) => void, personnel: User[], onNavigateBack: () => void }) => {
    ...
    const canSign = (user: User | null, personName: string | null | undefined): boolean => {
      ...
      return user.permissions.includes('Quality:Sign') || user.permissions.includes('Super User');
    };
                    ...
                    <CardContent>
                        {observations.map((item, index) => (
                            <div key={item.id} className="p-4 border rounded-lg mb-4">
                                ...
                                {item.photo && <Image src={issue.photo} alt={`Photo for ${item.text}`} width={200} height={112} className="mt-2 rounded-md" />}
                            </div>
                        ))}
                    </CardContent>
                </Card>
