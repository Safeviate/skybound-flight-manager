...
import type { AuditChecklist, ChecklistItemType, ChecklistCategory } from '@/lib/types';
...
const checklistTemplateFormSchema = z.object({
  ...
  category: z.custom<ChecklistCategory>(),
  items: z.array(checklistItemSchema).min(1, { message: "You must add at least one checklist item." }),
});

type ChecklistFormValues = z.infer<typeof checklistTemplateFormSchema>;
...
export function ChecklistTemplateForm({ onSubmit, existingTemplate }: ChecklistTemplateFormProps) {
  ...
  const handleFormSubmit = (data: ChecklistFormValues) => {
    const newChecklist = {
      ...data,
      items: data.items.map(item => ({ ...item, id: item.id || `item-${Date.now()}-${Math.random()}`, completed: false })),
    };
      ...
      <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
            ...
            render={({ field }) => (
                ...
                <FormControl><Input placeholder="e.g., C172 Pre-Flight" {...field} /></FormControl>
                <FormMessage />
                </FormItem>
            )}
            />
                ...
                <div key={field.id} className="flex items-start gap-2 p-2 border rounded-md">
                  <div className="flex-1 space-y-2">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormControl><Textarea placeholder={`Item #${index + 1} instructions`} {...field} className="min-h-[40px]" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
        ...
        <Button type="button" variant="outline" className="w-full" onClick={() => append({ text: '', type: 'Checkbox' })}>
